...
// function clampText(elem,numOfLines){
//     const pLer=document.querySelectorAll(`.${elem}`);
//     pLer.forEach(item=>{
//         const oneLine=parseFloat(getComputedStyle(item).lineHeight);//bir setrin hundurluyu
//         const maxHeight = oneLine * numOfLines;

//         if(item.scrollHeight>maxHeight){
//             let text=item.innerText;
//             while (item.scrollHeight > maxHeight && text.length > 0) {
//                 text = text.slice(0, -1);
//                 item.innerText = text + '...';
//             }
//         }
    ...
    copy.map(item=>{
        ...
        <article class="w-full row:w-[calc(50%-20px)] max-smallest2:p-[10px] p-[24px] bg-[#faf4ff] border-[2px] rounded-[30px] border-[#e4d7fd]">
                        <div class="w-full rounded-[15px] overflow-hidden">
                            <img class="w-full h-[200px] object-cover"  src="${item.image}" >
                        </div>
